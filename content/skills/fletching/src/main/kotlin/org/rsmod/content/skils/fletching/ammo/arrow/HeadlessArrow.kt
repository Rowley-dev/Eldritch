...
class HeadlessArrow : PluginScript() {
    ...
    private suspend fun ProtectedAccess.attachFeather(first: ObjType, second: ObjType) {
        ...
        while (setsToMake > 0) {
            ...
            val headlessArrow = headless_arrow_objs.headless_arrow as UnpackedObjType
            player.statAdvance(stats.fletching, headlessArrow.param(params.skill_xp).toDouble())
            setsToMake--
        }
    }
}
